...
jar and tell java to run it directly:
    ...
    $ sbt package-dist
    $ java -Dstage=development -jar ./dist/nbq/nbq-1.0.0-SNAPSHOT.jar
...
is named "nbq", there should be a `nbq.log` with contents like this:
...
The IDL for your service is in `src/main/thrift/nbq.thrift`.  The
...
In `src/main/scala`, take a look at `NbqServiceImpl.scala`. (This may
...
At the top of NbqServiceImpl.scala, add:
...
src/test/scala/com/twitter/nbq/NbqServiceSpec.scala.  Check
...
we just added.
    ...
    "verify stats" in {
      ...
      nbq.put("name", "bluebird")()
      nbq.get("name")() mustEqual "bluebird"
      counters.getOrElse("nbq.gets", 1) must_==1
      counters.getOrElse("nbq.puts", 1) must_==1
    }
...
command line.
    ...
    [info] Building project nbq 1.0.0-SNAPSHOT against Scala 2.8.1
    [info]    using NbqProject with sbt 0.7.4 and Scala 2.7.7
